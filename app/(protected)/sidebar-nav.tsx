...
function UserAvatar({ email }: { email: string }) {
  ...
  return (
    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-blue-500 to-blue-600 text-xs font-semibold text-white">
      {initials || '?'}
    </div>
  );
}
...
export function SidebarNav({ userEmail }: { userEmail: string }) {
  ...
  // On /chat the inner conversation list is already a sidebar, so collapse
  // the main app sidebar to icon-only mode even on wide screens. Reuses the
  // existing `md` breakpoint layout — no new state or components.
  const isChatRoute = pathname.startsWith('/chat');
  const labelClass = isChatRoute ? 'hidden' : 'lg:block md:hidden';
  const desktopAsideWidth = isChatRoute ? 'md:w-16' : 'md:w-16 lg:w-64';
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col">
      ...
      <div className="flex h-14 items-center gap-2.5 px-4">
        ...
        <span className={`text-sm font-semibold tracking-tight ${labelClass}`}>
          LaunchKit
        </span>
      </div>
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <aside
        className={`hidden shrink-0 border-r border-zinc-200/80 bg-white md:flex md:flex-col dark:border-zinc-800/80 dark:bg-zinc-950 ${desktopAsideWidth}`}
      >
        {sidebarContent}
      </aside>
    </>
